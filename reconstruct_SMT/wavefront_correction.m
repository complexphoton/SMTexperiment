...
function wavefront_correction(x,y,r_z,division_step,n_division,Z,zone_id,subvolume_id,directory_save)
    tic
    ...
    if division_step > 0
        ...
        k_big = load(""+directory_save+"k_"+prev_step+".mat").k;
        N_big = length(k_big);
    ...
    k = load(""+directory_save+"k_"+division_step+".mat").k;
    N = length(k);
    % Input and output k are:
