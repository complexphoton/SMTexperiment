...
function wavefront_correction_3D(x,y,r_z,n_slice,division_step,Z,zone_id,subvolume_id,n_sub,directory_save)
    ...
    if division_step > 0
        ...
        for slice_id = 1:n_slice
            r_slice = r_z((slice_id-1)*N_big+1:slice_id*N_big,:);
    ...
    % 2. The initial image
    psi_in = zeros(Nx,Ny,n_slice);
    for slice_id = 1:n_slice
        r_slice = r_k((slice_id-1)*N+1:slice_id*N,:);
        psi_in(:,:,slice_id) = reshape(finufft2d3(fy(:),fx(:),r_slice(:),1,1e-2,Y,X),Nx,Ny);
    end
    I_in = abs(psi_in).^2;
    ...
    for slice_id = 1:n_slice
        r_slice = r_k((slice_id-1)*N+1:slice_id*N,:);
        r_out_update((slice_id-1)*N+1:slice_id*N,:) = r_slice.';
        clear r_slice
    end
    clear r_k
    % To be more convenient when working with the input and output-updated
    % matrices, we define
    r_in = r_in_update; r_out = r_out_update;
    ...
    while delta_M > 0.05
        ...
        phi_in = Z*c_in;
        r_out_update = repmat(exp(1i*phi_in),n_slice,1).*r_out;
        ...
        for slice_id = 1:n_slice
            ...
            clear Psi_out_slice r_out_slice
        end
        ...
        phi_out = Z*c_out;
        r_in_update = repmat(exp(1i*phi_out),n_slice,1).*r_in;
    ...
    clear r_out r_in_update r_out_update
    
    % 5. Update the reflection matrix of the zone, save the matrix and the Zernike coefficients
    phi_in = Z*c_in; phi_out = Z*c_out;
    r_update =  repmat(exp(1i*phi_out),n_slice,1).*r_in.*exp(1i*phi_in.');   
    
    middle_slice = round(n_slice/2);
    r_middle = r_update((middle_slice-1)*N+1:middle_slice*N,:);
    psi = (finufft2d3(fy(:),fx(:),r_middle(:),1,1e-2,Y,X));
    I = abs(psi).^2;
