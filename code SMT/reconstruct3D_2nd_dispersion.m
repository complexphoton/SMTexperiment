...
function [I_2d_scan] = reconstruct3D_2nd_dispersion(list_x_im,list_y_im,list_z_im,x_im,y_im,z_im,overlap2xy,overlap2z,k,list_k0,list_amp,coef_n1,coef_n2,h1,z_mirror,n_division,directory_r,prefix,directory_save)
    ...
    Psi_2d_scan = single(zeros(length(x_im),length(y_im),length(z_im))); % Initialize the volumetric complex image
    Psi_seq_scan =  single(zeros(length(x_im),length(y_im),length(z_im))); % Initialize the volumetric complex image
    ...
    for i_freq = [1:n_freq]
        ...
        psi_s_2d = zeros(length(x_im),length(y_im),length(z_im)); % Initialize the single-frequency volumetric complex image
        ...
        for subvolume_id = 1:n_sub
            ...
            for zone_id = 1:n_zone
                ...
                r_zone = exp(1i*phi_out).*r_subvolume.*exp(1i*phi_in.');
                ...
                % And, correct dispersion
                psi_s_2d = psi_s_2d+psi_expand*exp(-1i*phase_list_2d(i_freq));
                psi_s_seq = psi_s_seq+psi_expand*exp(-1i*phase_list_seq(i_freq));
            end
        end
        % Sum over freq
        Psi_2d_scan = Psi_2d_scan+psi_s_2d;
        Psi_seq_scan = Psi_seq_scan+psi_s_seq;
        toc

    end
    % Finally, the volumetric image is
    I_2d_scan = abs(Psi_2d_scan).^2;
    I_seq_scan = abs(Psi_seq_scan).^2;
    ...
    for ii = 1:Nz_im
        ...
        if M == 0
            if ii ~= 1 & ii ~= Nz_im
                I_2d_scan(:,:,ii) = 1/2*(I_2d_scan(:,:,ii+1)+I_2d_scan(:,:,ii-1));
            end
        end
    end
    for ii = 1:Nz_im
        ...
        if M == 0
            if ii ~= 1 & ii ~= Nz_im
                I_seq_scan(:,:,ii) = 1/2*(I_seq_scan(:,:,ii+1)+I_seq_scan(:,:,ii-1));
            end
        end
    end
    
    save(""+directory_save+"./I_3D_with_overlap_"+n_sub+"_subvolumes_2nd_dispersion_2d_scan.mat",'I_2d_scan')
    save(""+directory_save+"./I_3D_with_overlap_"+n_sub+"_subvolumes_2nd_dispersion_seq_scan.mat",'I_seq_scan')
