...
function build_r_z_3D(list_z,list_amp,phase_list,directory_r,prefix,h1,z_mirror,coef_n1,coef_n2,list_k0,directory_save,k)
    %% 1. Setting
    ...
    z_mid_list = [];
    for id_sub = 1:length(list_z) % Find the middle z of each subvolume
        z_sub = list_z{id_sub};
    ...
    for i_freq = 1:n_freq
        ...
        r = repmat(r,length(list_z),1).*exp(1i*kron(z_mid_list,fz));
        
        % Compensate dispersion, normalize with laser amplitude then sum r
        % together 
        rz = rz+r*exp(-1i*phase_list(i_freq))/list_amp(i_freq);
    end
    
    % Save the r_z matrix of each subvolume and clear
    for id_sub = 1:length(list_z)
        r_z = rz((id_sub-1)*N+1:id_sub*N,:);
        save(""+directory_save+"./r_z_3D_"+id_sub+".mat",'r_z')
    end
    clear rz r_z
end
