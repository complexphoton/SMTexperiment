...
% Load the reflection matrices from
...
% List of laser amplitudes at different frequencies. Normalize each reflection matrix with the corresponding amplitudes
...
for i_freq = 1:n_freq
    ...
    psi_oct = finufft2d3(fy(:),fx(:),r(:),1,1e-2,Y(:),X(:));
    Psi(:,i_freq) = psi_oct;
end
...
for i_freq = 1:n_freq
    ...
    psi_oct_single_freq = reshape(exp(-1i*omega*z_oct_im/300*(2/cos(alpha))).*finufft2d3(fy(:),fx(:),r(:),1,1e-2,Y_im(:),X_im(:)),Ny_im,Nx_im,Nz_im);
...
%% Build intensity normalization mask
mask = zeros(Ny_im,Nx_im);
for ii = 1:Nz_im
    mask = mask+I_oct(:,:,ii);
end
...
%% Show image
I_oct_mask = I_oct./mask;
for ii = 1:7
    I_xy = I_oct_mask(:,:,round(ii*Nz_im/8));
    I_yz = I_oct_mask(:,round(ii*Nx_im/8),:); I_yz = I_yz(:); I_yz = reshape(I_yz,Ny_im,Nz_im);
