...
function build_r_z_3D_vrm(list_z_vrm,dz_vrm,list_amp,list_k0,directory_save,n_sub,n_slice,k)
    % 1. Setting
    ...
    z_mid_list = [];
    for id_sub = 1:n_sub % Find the middle z of each subvolume
        z_sub = list_z_vrm{id_sub};
    ...
    for id_sub = 1:n_sub
        ...
        z_center = z_mid_list(id_sub);
        list_z = linspace(-(n_slice-1)/2,(n_slice-1)/2,n_slice)*dz_vrm+z_center; 
        ...
        for i_freq = 1:n_freq
            ...
            r = repmat(r,length(list_z),1).*exp(1i*mod(kron(list_z,fz0),2*pi));
        ...
        save(""+directory_save+"./r_z_3D_class_"+id_sub+".mat",'rz')
        clear rz
    end
end
