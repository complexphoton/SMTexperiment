...
function wavefront_correction_vrm_3D(x,y,k,rz,n_slice,subvolume_id,directory_save)
    ...
    for slice_id = 1:n_slice
        psi_in_slice = finufft2d3(fy(:),fx(:),r_z(:),1,1e-2,Y,X);
    ...
    r_out_update = [];
    for slice_id = 1:n_slice
        r_slice = r_z((slice_id-1)*N+1:slice_id*N,(slice_id-1)*N+1:slice_id*N);
    ...
    while delta_M > 0.05
        ...
        Psi_in = [];
        for slice_id = 1:n_slice
            r_in_update_slice = r_in_update((slice_id-1)*N+1:slice_id*N,(slice_id-1)*N+1:slice_id*N);
            parfor ii = 1:N
        ...
        Psi_out = [];
        for slice_id = 1:n_slice
            r_out_update_slice = r_out_update((slice_id-1)*N+1:slice_id*N,(slice_id-1)*N+1:slice_id*N);
            parfor ii = 1:N
    ...
    save(""+directory_save+"phi_in_"+n_division+"_zone_"+zone_id+"_subvolume_"+subvolume_id+"_vrm.mat",'phi_in');
    save(""+directory_save+"phi_out_"+n_division+"_zone_"+zone_id+"_subvolume_"+subvolume_id+"_vrm.mat",'phi_out');
end
