...
for i_freq =  1:n_freq
    ...
    psi_ocm_single_freq = exp(-2*1i*omega*time).*finufft2d3(fy(:),fx(:),r(:).*exp(1i*fz(:)*zf),1,1e-2,Y_im(:),X_im(:))/list_amp(i_freq);
    psi_ocm = psi_ocm+reshape(psi_ocm_single_freq,Ny_im,Nx_im,Nz_im);
end

I_ocm = abs(psi_ocm).^2;
save(""+directory_save+"OCM.mat",'I_ocm')

%% Show image
n = 7;
for ii = 1:n
    I_xy = I_ocm(:,:,round(ii*Nz_im/(n+1)));
    I_yz = I_ocm(:,round(ii*Nx_im/(n+1)),:); I_yz = I_yz(:); I_yz = reshape(I_yz,Ny_im,Nz_im);

    figure(ii)
    imagesc(I_xy)
    axis image
    colormap('hot')
    caxis([0 0.5]*max(I,[],'all'))
