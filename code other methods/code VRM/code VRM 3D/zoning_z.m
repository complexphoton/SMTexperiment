...
% the z axis
function [list_z, list_z_im] = zoning_z(z,z_im,dz,dz_im,n_sub,overlap2)
    % Thickness of the image
    l = max(z,[],'all')-min(z,[],'all');
    % Half size of the overlap
    d = overlap2;
    list_z = cell(n_sub,1); list_z_im = cell(n_sub,1);
    for sub_id = 1:n_sub
        ...
        list_z{sub_id,1} = low_bound+d:dz:up_bound-dz/2-d;
